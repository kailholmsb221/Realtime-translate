...
| `autoclone.py` | `AutoCloner`: клон голоса собеседника из первых секунд разговора |
...
* **STT не ждёт перевод и синтез.** Распознавание крутится в основном цикле,
  ...
  логируется и теряет одну фразу, а не сессию. Обрыв самого источника
  (устройство пропало, таймаут WASAPI) тоже не поднимает исключение из задачи
  конвейера: он пишется в лог с полным traceback, растит `stats.errors` и
  останавливает только свой поток. То же у рассылки событий: сбой отправки
  одного конверта не убивает фоновую задачу WebSocket-рассылки.
...
* **Голоса.** `voice_id` из `session.start` — это голос **пользователя**, он
  озвучивает outbound (вашу речь для собеседника). Голос собеседника движок
  клонирует сам (см. ниже), inbound до готовности клона звучит встроенным
  голосом XTTS.
* **`tts.chunk` по умолчанию не уходит в UI** (служебное событие, трафик
  заметный). Включается `RT_WS_TTS_CHUNKS=1`.

## Голоса: свой и собеседника

| Чей голос | Откуда берётся | Где звучит |
|---|---|---|
| Пользователя | профиль создан заранее: `POST /api/voices` или `scripts/tts_smoke.py --create-voice`; `voice_id` приходит в `session.start` | outbound — перевод вашей речи уходит в Zoom этим голосом |
| Собеседника | **автоклон**: движок копит его речь в первые секунды звонка и создаёт профиль сам | inbound — перевод его речи звучит в ваших наушниках |

### Автоклон голоса собеседника

Реализация — `autoclone.py`, подключается только к потоку `in`:

1. конвейер отдаёт `AutoCloner` каждый чанк 16 kHz (окно последних 60 с);
2. по каждому `stt.final` из окна вырезается **речевой** участок
   `[t_start_ms, t_end_ms]` — паузы и шум в сэмпл не попадают;
3. набрав `RT_AUTO_CLONE_MIN_S` (по умолчанию 12) секунд чистой речи, движок в
   фоне (`asyncio.to_thread`, конвейер не ждёт) создаёт профиль через
   `VoiceStore.create_voice`; латенты считает `XttsProvider.compute_latents`
   (тот же путь, что у `scripts/tts_smoke.py --create-voice`), в фейковом
   режиме латентов нет — XTTS посчитал бы их при первом синтезе;
4. профиль пишется в таблицу `voices`, имя — `auto_session<id>_in`; префикс
   `auto_` и есть признак автопрофиля (схему БД менять нельзя);
5. следующие фразы inbound синтезируются уже этим голосом
   (`Pipeline.voice_id`), в сэмпл больше ничего не копится;
6. по `session.stop` профиль удаляется с диска и из таблицы, если
   `RT_AUTO_CLONE_KEEP=0` (по умолчанию).

Для казахского автоклон не создаётся: XTTS клонирует только ru/en, а
kk-провайдер `voice_id` игнорирует. Механизм целиком выключается
`RT_AUTO_CLONE=0`.
...
уведомить его — обязанность пользователя (UI показывает статус записи). Именно
поэтому автопрофили по умолчанию живут только на время сессии, а выключить
автоклон можно одной переменной.
...
| `RT_AUTO_CLONE` | `1` | `0` — не клонировать голос собеседника |
| `RT_AUTO_CLONE_MIN_S` | `12` | сколько секунд чистой речи накопить до клона (минимум 5) |
| `RT_AUTO_CLONE_MAX_S` | `30` | сколько секунд максимум класть в сэмпл (максимум 60) |
| `RT_AUTO_CLONE_KEEP` | `0` | `1` — оставлять автопрофиль на диске после сессии |
...
| Голос собеседника не клонировался | в логе смотрите `autoclone`: не набралось `RT_AUTO_CLONE_MIN_S` секунд речи, язык kk или `RT_AUTO_CLONE=0` |
| Автопрофили копятся в `voices/` | по умолчанию удаляются; проверьте `RT_AUTO_CLONE_KEEP` |
...
Замечание: `mypy engine/orchestrator` проверяет и импортируемые модули; сейчас
и он, и `mypy engine` целиком проходят без ошибок.
...
* `test_orchestrator_pipeline.py` — из фикстуры «тон — тишина — тон» получаются
  ...
  рекордера, устойчивость к падению перевода и к обрыву источника (задача
  конвейера завершается штатно, без утёкшего исключения); автоклон — профиль
  создаётся ровно один раз, последующие фразы синтезируются новым `voice_id`, в сэмпл
  попадает только речь, `RT_AUTO_CLONE=0` отключает механизм, при остановке
  профиль удаляется (KEEP=0) или остаётся (KEEP=1);
* `test_orchestrator_server.py` — настоящие серверы на свободных портах:
  ...
  рассылка переживает сбой отправки одного события;
  автоклон подключён только к inbound, `voice_id` пользователя — только к
  outbound, для kk автоклон не создаётся;
...
* **Ассистент-LLM** (ARCHITECTURE.md этап 5): сейчас движок отдаёт только живой
  ...
  владельца отложено.
* **`stt.final` без `utterance_id`** (решение владельца: контракт не меняем).
  UI связывает перевод с репликой по порядку и `src_text`; движок гарантирует
  порядок и `ref_utterance_id`. Если две одинаковые фразы подряд начнут путать
  ленту — вернуться к вопросу о поле `utterance_id` в `stt.final`.
