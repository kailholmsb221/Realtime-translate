...
- отбрасываются артефакты whisper: заученные «титры», подписи звуков
  («СПОКОЙНАЯ МУЗЫКА», «СТОН!» — модель так помечает не-речь), залипания вида
  «No, no, no…» ×100; ответ NLLB, оставшийся на языке оригинала, не озвучивается.
